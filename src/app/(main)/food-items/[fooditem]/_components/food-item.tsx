...
export function FoodItem(props: {foodItem: NonNullable<FoodItemType>}) {
  let {foodItem} = props;
  return (
...
function MacroNutrients(props: {foodItem: NonNullable<FoodItemType>}) {
  let {foodItem} = props;
  let micronutrientsInPercentage = calculateMicronutriensInPercentage(
    foodItem.nutrition
  );
  return (
    <div className="mb-5 flex flex-col gap-2">
      ...
      >
        <div className="flex h-3 w-full overflow-hidden rounded-lg bg-chart-1 shadow-xl">
          <div
            className={cn(
              "h-full",
              getClassNameBasedOnMicronutrient(
                micronutrientsInPercentage.protein
              )
            )}
          />
          <div
...
function NutritionInfo(props: {foodItem: NonNullable<FoodItemType>}) {
  let {foodItem} = props;
  return (
...
function Header(props: {foodItem: NonNullable<FoodItemType>}) {
  let {foodItem} = props;
  return (
    <CardHeader>
...
function getClassNameBasedOnMicronutrient(nutrient: number) {
  if (nutrient < 10) {
    return "w-[10%] bg-chart-5";
  }
  if (nutrient < 20) {
    return "w-[20%] bg-chart-5";
  }
  if (nutrient < 30) {
    return "w-[30%] bg-chart-4";
  }
  if (nutrient < 40) {
    return "w-[40%] bg-chart-4";
  }
  if (nutrient < 50) {
    return "w-[50%] bg-chart-3";
  }
  if (nutrient < 60) {
    return "w-[60%] bg-chart-3";
  }
  if (nutrient < 70) {
    return "w-[70%] bg-chart-3";
  }
  if (nutrient < 80) {
    return "w-[80%] bg-chart-3";
  }
  if (nutrient < 90) {
    return "w-[90%] bg-chart-1";
  }
  return "w-full bg-chart-1";
}
